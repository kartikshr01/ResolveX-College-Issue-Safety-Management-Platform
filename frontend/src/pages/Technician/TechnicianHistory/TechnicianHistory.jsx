...
import { getTechnicianHistory } from "../../../services/ticket.service";
...
function TechnicianHistory() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        // API response may be returned directly or inside data
  ...
  if (error) {
    return (
      <div className="technician-history-page">
        <div className="technician-history-error">{error}</div>
      </div>
    );
  }
  ...
  return (
    <div className="technician-history-page">
      <div className="technician-history-header">
        <div>
          <p className="technician-history-eyebrow">TECHNICIAN RECORDS</p>

          <h1>Resolution History</h1>

          <p className="technician-history-subtitle">
            View issues that you have successfully resolved.
          </p>
        </div>
        ...
        <div className="technician-history-count">
          ...
          <span>
            {tickets.length === 1 ? "Resolved" : "Resolved Tickets"}
          </span>
        </div>
      </div>
      ...
      {tickets.length === 0 ? (
        <div className="technician-history-empty">
          <div className="technician-history-empty-icon">✓</div>

          <h2>No Resolved Issues</h2>

          <p>Your resolved tickets will appear here.</p>
        </div>
      ) : (
            ...
            >
              <div className="technician-history-image">
                {ticket.imageUrl ? (
                  <img
                    src={ticket.imageUrl}
                    alt={ticket.title || "Resolved issue"}
                  />
                ) : (
